...
1. Run the following command to update files to use the name of your provider (third-party: use your GitHub organization/username):
   ...
   - find and replace all instances of the boilerplate `btp` with the `NAME` of your provider.
...
- binary: pulumi-resource-btp
  ...
  ignore: []
  ldflags:
  - -X github.com/pulumi/pulumi-btp/provider/pkg/version.Version={{.Tag}}
  main: ./cmd/pulumi-resource-btp/
  sort: asc
  use: git
release:
  disable: false
...
├── config.yaml
├── metadata.json
├── pulumi-btp_darwin_amd64_v1
│   └── pulumi-resource-btp
├── pulumi-btp_darwin_arm64
│   └── pulumi-resource-btp
├── pulumi-btp_linux_amd64_v1
│   └── pulumi-resource-btp
├── pulumi-btp_linux_arm64
│   └── pulumi-resource-btp
├── pulumi-btp_windows_amd64_v1
│   └── pulumi-resource-btp.exe
└── pulumi-btp_windows_arm64
    └── pulumi-resource-btp.exe
```
